...
import { CatchAsyncError, AsyncController } from "../../utils/errors/async.handler";
...
export const createUserController = AsyncController(async (req: Request) => {
  ...
  if (!createUserDto.email || !createUserDto.password || !createUserDto.firstName || !createUserDto.lastName) {
    throw new ErrorHandler("Missing required fields: email, password, firstName, lastName", 400);
  }

  // Check if user exists
  const existing = await userRepo.findByEmail(createUserDto.email);
  if (existing) {
    Logger.warn(`Attempt to create duplicate user with email: ${createUserDto.email}`);
...
export const updateUserController = AsyncController(async (req: Request) => {
  ...
  if (updateData.password) {
    throw new ErrorHandler("Password updates must go through the password reset endpoint", 400);
  }
